...
class TestLoadNEOs(unittest.TestCase):
    """
    @calssmethod的作用：
    很多博客只是说@calssmethod的作用就是“可以不需要实例化，直接类名.方法名()来调用。
    这有利于组织代码，把某些应该属于某个类的函数给放到那个类里去，同时有利于命名空间的整洁”。
    很抽象，其实具体作用如下：

    @classmethod的作用实际是可以在class内实例化class，一般使用在有工厂模式要求时。
    作用就是比如输入的数据需要清洗一遍再实例化，可以把清洗函数定义在class内部并
    加上@classmethod装饰器已达到减少代码的目的。总结起来就是：@class method可以用
    来为一个类创建一些预处理的实例。
    """
    @classmethod
    def setUpClass(cls):
